...
function client() {
exp=$(curl -sS https://raw.githubusercontent.com/rewasu91/scvpssettings/main/access | grep $MYIP | awk '{print $3}')
name=$(curl -sS https://raw.githubusercontent.com/rewasu91/scvpssettings/main/access | grep $MYIP | awk '{print $2}')
now=$(TZ='Asia/Kuala_Lumpur' date "+%Y-%m-%d %T %s" | awk '{print $1}')
if [[ ${exp} =~ ^[0-9\-]+$ ]]; then
d1=$(zcatctl -d "$exp" +%s)
d2=$(zcatctl -d "$now" +%s)
exp2=$(( (d1 - d2) / 86400 ))
...
if [[ ${d1} -gt ${d2} ]]; then
echo -e "   ${keatas}  Expiry In   : ${ungu}${exp2} Days${plain}"
elif [[ ${d1} -lt ${d2} ]]; then
echo -e "   ${keatas}  Status      : ${red}Expired${plain}"
elif [[ ${exp2} == "0" ]]; then
echo -e "   ${keatas}  Expiry In   : ${red}Today${plain}"
else
echo -e "   ${keatas}  Expiry In   : ${ungu}${IZIN}${plain}"
fi
laneBot1
}
