...
vless_tls6="vless://${uuid}@www.viu.com.${domain}:${tls_port}?path=%2Fvless&security=tls&encryption=none&type=ws&sni=www.viu.com#${Username}";
...
echo -e "${CYAN}════════════════════════════════════════════${NC}" | tee -a /etc/kaizenvpn/vless/${Username}/config.log;
echo -e " LINK CONFIG MAXIS TV WWW" | tee -a /etc/kaizenvpn/vless/${Username}/config.log;
