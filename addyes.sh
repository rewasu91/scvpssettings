...
# // Make Config Link
vless_nontls="vless://${uuid}@cdn.who.int:${nontls_port}?path=wss%3A//cdn.who.int/vless&security=none&encryption=none&host=${domain}&headerType=none&type=ws&quicSecurity=none#${Username}";
...
echo -e "${CYAN}════════════════════════════════════════════${NC}" | tee -a /etc/kaizenvpn/vless/${Username}/config.log;
echo -e " LINK CONFIG YES (VLESS WS NONE TLS)" | tee -a /etc/kaizenvpn/vless/${Username}/config.log;
