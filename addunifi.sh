...
# // Make Config Link
vless_nontls="vless://${uuid}@playtv.unifi.com.my.${domain}:${nontls_port}?path=%2Fvless&security=none&encryption=none&host=playtv.unifi.com.my&type=ws#${Username}";
vless_tls="vless://${uuid}@playtv.unifi.com.my.${domain}:${tls_port}?path=%2Fvless&security=tls&encryption=none&type=ws&sni=playtv.unifi.com.my#${Username}";
...
echo -e "${CYAN}════════════════════════════════════════════${NC}" | tee -a /etc/kaizenvpn/vless/${Username}/config.log;
echo -e " LINK CONFIG UNIFI TLS 443" | tee -a /etc/kaizenvpn/vless/${Username}/config.log;
echo -e " ${vless_tls}" | tee -a /etc/kaizenvpn/vless/${Username}/config.log;
echo -e "${CYAN}════════════════════════════════════════════${NC}" | tee -a /etc/kaizenvpn/vless/${Username}/config.log;
echo -e " LINK CONFIG UNIFI NON TLS 80" | tee -a /etc/kaizenvpn/vless/${Username}/config.log;
